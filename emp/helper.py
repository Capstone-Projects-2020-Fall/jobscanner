def get_best_match(myCV):
    ...
    gds = pd.read_json('https://www.dropbox.com/s/ec1kv4iu4m845bo/Resume%20Parser%20-%20Sheet1.json?dl=1',
                       orient='records')
    ...
    def cal_similarity(cv):
        similarity = []
        for skills in gds.Experience:
            similarity.append(Jaccard(cv, skills))

        gds['similarity'] = similarity
        col = ['Name', 'Skills', 'Phone', 'similarity']
